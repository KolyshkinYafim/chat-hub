...
describe("palette entry for failing checks", () => {
  it("offers the command only when the active session has failures", () => {
    const withFailures = buildPaletteEntries([session()], "", 0, 0, false, 2)
    ...
    const without = buildPaletteEntries([session()], "", 0, 0, false, 0)
  ...
  it("is found by typing about failing checks", () => {
    const entries = buildPaletteEntries([session()], "failing checks", 0, 0, false, 1)

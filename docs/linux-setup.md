...
**Проверено:** 2026-09-11, `main` `35af1a9`. Живой прогон на CachyOS, KDE Plasma 6, Wayland,
Node 22.23.2 (nvm) / pnpm 11.15.1; первый прогон — 2026-08-27 на `5883232`.

**Статус.** Приложение на Linux **запущено и работает**: окно открывается, дев-сервер отдаёт
рендерер, unix-сокеты поднимаются. С `feat/linux-build` есть framed-окна, Ctrl-хинты и сборка
`pnpm dist:linux` (AppImage + deb); CI гоняет `lint`, `typecheck`, тесты и Playwright E2E на
`ubuntu-latest`. Первый живой прогон (2026-08-27) вскрыл две вещи, которых по коду видно не было,
обе описаны ниже:
